...
* **Output grids for regridded data must also be registered** via the smae online form used to descibe the native computational grids, as described in the [EMD documenation](https://emd.mipcvs.dev/docs/Information_for_Submitters/Submission-Guide/) (i.e., for any grid used to report data that is not the model's native computional grid). It is recommended to do this at the same time as creation of the general EMD documatation (rather than afterwards), and remember that grid(s) for all archived variables must have been described prior to data publication, so that their grid labels (e.g. `g###`) can be included in the file metadata and file names. 
  
  !!! tip "EMD Tools"
      - **View your registered model**: [EMD Model Documentation](https://wcrp-cmip.github.io/Essential-Model-Documentation/docs/Model/)
      - **Track submission progress**: [Progress Tracker](https://wcrp-cmip.github.io/Essential-Model-Documentation/docs/Progress_Tracker.html) (enter your GitHub ID)
