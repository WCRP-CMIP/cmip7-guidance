---
layout: default
title: "Experiment Setup and Forcings Guidance: scen7-vl-Aer"
---

# Experiment Setup and Forcings Guidance: scen7-vl-Aer

Used to diagnose climate responses to the regionally heterogeneous evolution of anthropogenic aerosol emissions.
Anthropogenic aerosols and aerosol precursor emissions (BC, OC, NH3, SO2) evolve as in `scen7-h`.
All other forcings evolve as in `scen7-vl`.
Intended for models without interactive chemistry.
Models with interactive chemistry should run `scen7-vl-AQ` instead.
...
- [scen7-vl-AQ](./scen7-vl-aq.md) is the corresponding interactive-chemistry experiment for models that include
  interactive chemistry.

## Experiment set up

The `scen7-vl-Aer` simulation is a variant of the `scen7-vl` simulation.

The aerosol and tropospheric non-methane ozone precursor emissions should evolve as in the `scen7-h` simulation.

All other forcings should evolve as in `scen7-vl`.

`scen7-vl-Aer` is for models that do not include interactive chemistry.
...
`scen7-vl-Aer` branches from the [historical](./historical.md) simulation (part of [CMIP](./index.md#cmip)).
...
The `scen7-vl-Aer` experiment is a time-varying forcings experiment, combining aerosol and tropospheric non-methane
ozone precursor emissions from the `scen7-h` simulation with all other forcings from the `scen7-vl` simulation.
...
For all other forcings, the forcing versions relevant for this simulation are the same as for the `scen7-vl` simulation.
...
EXPERIMENT_NAME="scen7-vl-Aer"

esgpull add --track --tag ${EXPERIMENT_NAME} source_id:IIASA-IAMC-h-1-0-0,IIASA-IAMC-1-0-0,IIASA-IAMC-vl-1-0-0,UofMD-landState-vl-3-1-1,CR-vl-1-0-0,UOEXETER-ScenarioMIP-2-2-2,FZJ-CMIP-ozone-h-1-0,FZJ-CMIP-nitrogen-h-1-0,SOLARIS-HEPPA-ScenarioMIP-4-6,PIK-vl-1-0-0
