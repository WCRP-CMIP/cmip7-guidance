...
def items_to_nav(items, nav_lines, docs_root, indent="", base_path=None):
    ...
    for item in sorted(items, key=lambda x: x["sort"]):
        if item["type"] == "file":
            if base_path.name == "Experiment_set_up_and_Forcings":
                # Don't clean hyphens in the name
                title = clean_title_file(item["name"], replace_by_whitespace=("_",))
                # Could use esgvoc to get the DRS name for the title here
...
def generate_navigation(docs_path):
    ...
    items_to_nav(
        other_items, nav_lines, indent="", base_path=docs_path, docs_root=docs_path
    )
